...
public class OrbitateAction implements RulesAction {
	...
	public MOVE execute(Game game) {
        if (!game.doesGhostRequireAction(ghost)){ 	//if it does not requires an action
        	return MOVE.NEUTRAL;
        }
        
       //Checks the new value of the limit distance
      //Initial distance is 101 because 100 is the limit when pacMan can eat us before die so in the end when edible time is 0 the limit will be 1
       this.orbitDistance = (game.getGhostEdibleTime(this.ghost)*2) + 1; 
