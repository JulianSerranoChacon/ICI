...
(defrule PINKYpacmanFarAway
   ...
   (test (or (!= ?t 0) (> ?d (+ (/ ?e 2) 1))))
	=>
   (assert (ACTION (id PINKYOrbit) (info "PINKY far away and edible") (priority 21)))
)
...
(defrule PINKYhayEscudero
	...
	(test (> ?e 0))
	=>
	(assert (ACTION (id RunToEscuderoAction) (info "PINKY going to escudero") (extraGhost ?g) (priority 20)))
)
...
(defrule PINKYpacmanNear
   ...
   (test (or (== ?t 0) (< ?d 200)))
	=>
   (assert (ACTION (id PINKYrunsOptimal) (info "PINKY near and edible") (priority 19)))
)
...
(defrule PINKYrunsAwayMSPACMANclosePPill
	...
	(test (<= ?d 30))
	=>
	(assert (ACTION (id "PINKYstartRunning") (info "MSPacMan cerca PPill") (priority 18)))
)
...
(defrule PINKYSecondNearestToMsPacman
	...
	(test
		(or
			(and (> ?pinkyDistance ?inkyDistance)
...
(defrule PINKYBlinkingAndSafe
	(PINKYtoPacman (distanceTo ?pinkyDistance))
	(PINKYedible (edibleTime ?e))
	(test (> ?pinkyDistance 40))
	(test (< ?e 10))
	=>
